...
class AttendenceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.hr.attendence.ajax.index", compact('data', 'users'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.attendence.ajax_index", compact('data', 'users'));
    }

    public function create(Request $request)
    {
        if ($request->ajax()) {
            $html = view("dftqc.hr.attendence.ajax.create")->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.attendence.create");
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.hr.attendence.ajax.show", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.attendence.show", compact('data'));
    }
    ...
    public function edit(Request $request, $id)
    {
        $data = Attendence::findOrFail($id);
        if ($request->ajax()) {
            $html = view("dftqc.hr.attendence.ajax.edit", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.attendence.edit", compact('data'));
    }
    ...
    public static function getAjaxContent($type, $id = '', $option = '')
    {
        switch ($type) {
            case 'index':
                ...
                return view("dftqc.hr.attendence.ajax.index", compact('data'))->render();
                break;
            case 'create':
                return view("dftqc.hr.attendence.ajax.create")->render();
                break;
            case 'edit':
                $data = Attendence::findOrFail($id);
                return view("dftqc.hr.attendence.ajax.edit", compact('data'))->render();
                break;
            default:
                return 'Not Found';
        }
    ...
    public function api(Request $request, $action, $authCode = null)
    {
        ...
        if ($route[0] == 'api') {
            switch ($action) {
                case 'index':
                    $data = Attendence::where('status', '<>', -1)->get();
                    $html = view("dftqc.hr.attendence.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                    break;
                case 'store':
                    Attendence::create($request->all());
