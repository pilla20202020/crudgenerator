...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.hr.employee.ajax.index", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.employee.ajax_index", compact('data'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $html = view("dftqc.hr.employee.ajax.create", ['emp_id' => $emp_id])->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.employee.create");
    }
    ...
    public function show(Request $request, $id)
    {
      ...
      //  $documents = Document::where('imageable_type', Employee::class)->where('imageable_id', $data->employee_id)->get();
        if ($request->ajax()) {
            $html = view("dftqc.hr.employee.ajax.show", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        //return view("dftqc.hr.employee.show", compact('data', 'documents'));
        return view("dftqc.hr.employee.show", compact('data'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.hr.employee.ajax.edit", compact('data', 'skills'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.hr.employee.edit", compact('data', 'skills'));
    }
    ...
    public static function getAjaxContent($type, $id = '', $option = '')
    {
        switch ($type) {
            case 'index':
                ...
                return view("dftqc.hr.employee.ajax.index", compact('data'))->render();
                break;
            case 'create':
                return view("dftqc.hr.employee.ajax.create")->render();
                break;
            case 'edit':
                $data = Employee::findOrFail($id);
                return view("dftqc.hr.employee.ajax.edit", compact('data'))->render();
                break;
            default:
                return 'Not Found';
        }
    ...
    public function api(Request $request, $action, $authCode = null)
    {
        ...
        if ($route[0] == 'api') {
            switch ($action) {
                ...
                case 'edit':
                    $data = Employee::findOrFail($id);
                    $html = view("dftqc.hr.employee.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                    break;
                case 'update':
                    $data = Employee::findOrFail($id);
