           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.hr.designation.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.designation.ajax_index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.hr.designation.ajax.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.designation.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.hr.designation.ajax.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.designation.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = Designation::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.hr.designation.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.designation.edit", compact('data'));
            }
            ...
            public static function getAjaxContent($type, $id = '', $option = '')
            {
                switch ($type) {
                    case 'index':
                        ...
                        return view("dftqc.hr.designation.ajax.index", compact('data'))->render();
                        break;
                    case 'create':
                        return view("dftqc.hr.designation.ajax.create")->render();
                        break;
                    case 'edit':
                        $data = Designation::findOrFail($id);
                        return view("dftqc.hr.designation.ajax.edit", compact('data'))->render();
                        break;
                    default:
                        return 'Not Found';
                }
