           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.hr.employeeonboarding.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.employeeonboarding.ajax_index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.hr.employeeonboarding.ajax.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.employeeonboarding.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.hr.employeeonboarding.ajax.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.employeeonboarding.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = Employeeonboarding::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.hr.employeeonboarding.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.hr.employeeonboarding.edit", compact('data'));
            }
            ...
            public static function getAjaxContent($type, $id = '', $option = '')
            {
                switch ($type) {
                    case 'index':
                        ...
                        return view("dftqc.hr.employeeonboarding.ajax.index", compact('data'))->render();
                        break;
                    case 'create':
                        return view("dftqc.hr.employeeonboarding.ajax.create")->render();
                        break;
                    case 'edit':
                        $data = Employeeonboarding::findOrFail($id);
                        return view("dftqc.hr.employeeonboarding.ajax.edit", compact('data'))->render();
                        break;
                    default:
                        return 'Not Found';
                }
