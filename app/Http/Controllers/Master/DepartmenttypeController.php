           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.departmenttype.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.departmenttype.ajax_index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.master.departmenttype.ajax.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.departmenttype.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.departmenttype.ajax.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.departmenttype.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = Departmenttype::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.master.departmenttype.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.departmenttype.edit", compact('data'));
            }
            ...
            public static function getAjaxContent($type, $id = '', $option = '')
            {
                switch ($type) {
                    case 'index':
                        ...
                        return view("dftqc.master.departmenttype.ajax.index", compact('data'))->render();
                        break;
                    case 'create':
                        return view("dftqc.master.departmenttype.ajax.create")->render();
                        break;
                    case 'edit':
                        $data = Departmenttype::findOrFail($id);
                        return view("dftqc.master.departmenttype.ajax.edit", compact('data'))->render();
                        break;
                    default:
                        return 'Not Found';
                }
