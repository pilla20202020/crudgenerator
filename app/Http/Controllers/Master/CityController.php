           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.city.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.city.ajax_index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.master.city.ajax.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.city.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.city.ajax.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.city.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = City::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.master.city.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.city.edit", compact('data'));
            }
            ...
            public static function getAjaxContent($type, $id = '', $option = '')
            {
                switch ($type) {
                    case 'index':
                        ...
                        return view("dftqc.master.city.ajax.index", compact('data'))->render();
                        break;
                    case 'create':
                        return view("dftqc.master.city.ajax.create")->render();
                        break;
                    case 'edit':
                        $data = City::findOrFail($id);
                        return view("dftqc.master.city.ajax.edit", compact('data'))->render();
                        break;
                    default:
                        return 'Not Found';
                }
