           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.nationality.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.nationality.ajax_index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.master.nationality.ajax.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.nationality.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.master.nationality.ajax.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.nationality.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = Nationality::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.master.nationality.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.master.nationality.edit", compact('data'));
            }
            ...
            public static function getAjaxContent($type, $id = '', $option = '')
            {
                switch ($type) {
                    case 'index':
                        ...
                        return view("dftqc.master.nationality.ajax.index", compact('data'))->render();
                        break;
                    case 'create':
                        return view("dftqc.master.nationality.ajax.create")->render();
                        break;
                    case 'edit':
                        $data = Nationality::findOrFail($id);
                        return view("dftqc.master.nationality.ajax.edit", compact('data'))->render();
                        break;
                    default:
                        return 'Not Found';
                }
