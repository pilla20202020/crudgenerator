...
class GeneralFormController extends Controller
{
    public function create()
    {
        ...
        try {
            ...
            return view('dftqc.form.create', compact('allTables'));
        } catch (Exception $e) {
            dd($e->getMessage());
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        switch ($command) {
                ...
                if ($tableName != "") {
                    switch ($tableName) {
                        case 'add':
                            ...
                            return view("dftqc.settings.dictonary/add", $data);
                        default:
                            $data['TableRows'] = DB::select("select * from " . $data['tableName']);
                            return view("dftqc.settings.dictonary.list", $data);
                    }
                }
                $data['TableName'] = "tbl_dictonary";
                $data['TableRows'] = DB::select("select * from " . $data['TableName']);
                return view("dftqc.settings.dictonary", $data);
            ...
            case 'curd':
                ...
                return view("dftqc.settings.curd", $data);
                break;
            case 'ajax-curd':
                $data['tableName'] = $tableName;
                $data['directoryName'] = $directoryName;
                ...
                BibClass::ajaxShowContent($tableName, $directoryName);
                return view("dftqc.settings.ajax-curd", $data);
                break;
            ...
            default:
                ...
                return view("dftqc.settings.home");
        }
    }

    public function getTableNullablecreate()
    {
        return view("dftqc.settings.ajax-curd");
    }
}
