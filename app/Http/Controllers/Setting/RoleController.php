...
class RoleController extends Controller
{
    public function index(Request $request)
    {
        ...
        // dd($data);
        if ($request->ajax()) {
            $html = view("dftqc.setting.role.ajax.index", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.setting.role.ajax_index", compact('data'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.setting.role.ajax.create", ['modules' => $modules])->render();
            return response()->json(['status' => true, 'content' => $html], 200);

        }
        return view("dftqc.setting.role.create", ['modules' => $modules]);
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.setting.permission.ajax.show", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.setting.role.show", compact('data'));
    }
    ...
    public function edit(Request $request, $id)
    {
        $role = Role::findOrFail($id);
            ...
            $modules = Module::all()->pluck('module_id')->toArray();
        if ($request->ajax()) {
            $html = view("dftqc.setting.role.ajax.edit", ['modules' => $modules, 'role' => $role])->render();

            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.setting.role.edit", compact('data'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('setting.role.index')->with('success', 'The Role updated Successfully.');
    }
    ...
    public static function getAjaxContent($type, $id = '', $option = '')
    {
        switch ($type) {
            case 'index':
                ...
                return view("dftqc.setting.permission.ajax.index", compact('data'))->render();
                break;
            case 'create':
                return view("dftqc.setting.permission.ajax.create")->render();
                break;
            case 'edit':
                $data = User::findOrFail($id);
                return view("dftqc.setting.permission.ajax.edit", compact('data'))->render();
                break;
            default:
                return 'Not Found';
        }
    ...
    public function api(Request $request, $action, $authCode = null)
    {
        ...
        if ($route[0] == 'api') {
            switch ($action) {
                case 'index':
                    $data = User::where('status', '<>', -1)->get();
                    $html = view("dftqc.setting.permission.ajax.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                    break;
                case 'store':
                ...
                case 'edit':
                    $data = User::findOrFail($id);
                    $html = view("dftqc.setting.permission.ajax.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                    break;
                case 'update':
                    $data = User::findOrFail($id);
