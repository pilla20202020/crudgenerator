           ...
           public function index(Request $request)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.transfer.index", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.transfer.index", compact('data'));
            }

            public function create(Request $request)
            {
                if ($request->ajax()) {
                    $html = view("dftqc.transfer.create")->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.transfer.create");
            }
            ...
            public function show(Request $request, $id)
            {
                ...
                if ($request->ajax()) {
                    $html = view("dftqc.transfer.show", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.transfer.show", compact('data'));
            }
            ...
            public function edit(Request $request, $id)
            {
                $data = Transfer::findOrFail($id);
                if ($request->ajax()) {
                    $html = view("dftqc.transfer.edit", compact('data'))->render();
                    return response()->json(['status' => true, 'content' => $html], 200);
                }
                return view("dftqc.transfer.edit", compact('data'));
            }
            ...
            public function api(Request $request, $action, $authCode = null)
            {
                ...
                if ($route[0] == 'api') {
                    switch ($action) {
                        case 'index':
                            $data = Transfer::where('status', '<>', -1)->get();
                            $html = view("dftqc.ajax.transfer.index", compact('data'))->render();
                            return response()->json(['status' => true, 'content' => $html], 200);
                            break;
                        case 'store':
                            Transfer::create($request->all());
                        ...
                        case 'edit':
                            $data = Transfer::findOrFail($id);
                            $html = view("dftqc.ajax.transfer.edit", compact('data'))->render();
                            return response()->json(['status' => true, 'content' => $html], 200);
                            break;
                        case 'update':
                            $data = Transfer::findOrFail($id);
