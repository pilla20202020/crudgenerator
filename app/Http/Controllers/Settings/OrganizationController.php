...
class OrganizationController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.settings.organization.ajax.index", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.settings.organization.ajax_index", compact('data'));
    }

    public function create(Request $request)
    {
        if ($request->ajax()) {
            $html = view("dftqc.settings.organization.ajax.create")->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.settings.organization.create");
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            $html = view("dftqc.settings.organization.ajax.show", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.settings.organization.show", compact('data'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // dd($data);
        if ($request->ajax()) {
            $html = view("dftqc.settings.organization.ajax.edit", compact('data'))->render();
            return response()->json(['status' => true, 'content' => $html], 200);
        }
        return view("dftqc.settings.organization.edit", compact('data'));
    }
    ...
    public static function getAjaxContent($type, $id = '', $option = '')
    {
        switch ($type) {
            case 'index':
                ...
                return view("dftqc.settings.organization.ajax.index", compact('data'))->render();
                break;
            case 'create':
                return view("dftqc.settings.organization.ajax.create")->render();
                break;
            case 'edit':
                $data = Organization::findOrFail($id);
                return view("dftqc.settings.organization.ajax.edit", compact('data'))->render();
                break;
            default:
                return 'Not Found';
        }
